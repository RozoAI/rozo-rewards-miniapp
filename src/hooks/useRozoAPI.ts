...
const ERROR_MESSAGES = {
  ...
  [ErrorType.INSUFFICIENT_AUTHORIZATION]:
    "Insufficient spending authorization. Please increase your limit.",
  ...
  [ErrorType.SIGNATURE_REJECTED]:
    "Signature was rejected. Please try signing again.",
...
export const useRozoAPI = () => {
  ...
  const authenticateWallet = useCallback(async () => {
    ...
    try {
      ...
      const nonce = `auth_${Date.now()}_${Math.random()
        .toString(36)
        .substr(2, 9)}`;
        ...
        headers: {
          ...
          Origin:
            typeof window !== "undefined"
              ...
              : "https://rozo-rewards-miniapp.vercel.app",
        },
        mode: "cors",
        credentials: "omit",
        body: JSON.stringify({
      ...
      if (
        ...
        toast.error(
          "Signature cancelled. Authentication is required for ROZO features."
        );
        return null;
        ...
        if (
          ...
          console.warn(
            "API unreachable, using development fallback authentication"
          );
          ...
          toast.success(
            "🔧 Development mode: Authentication simulated successfully!"
          );
          return devToken;
        }
      ...
      try {
        ...
        if (!response.ok) {
          ...
          const isAuthError =
            ...
            (errorData.error?.message &&
              ...
              errorData.error.message.includes("401")) ||
            (errorData.error?.code &&
              typeof errorData.error.code === "string" &&
              errorData.error.code.includes("token"));

          if (isAuthError) {
            console.log(
              "API returned authentication error, clearing authentication state"
            );
            localStorage.removeItem("rozo_jwt_token");
            localStorage.removeItem("rozo_jwt_expires");
            setAuthToken(null);
        ...
        if (
          ...
          try {
            ...
            if (decoded.dev_mode) {
              console.warn(
                `API call to ${endpoint} failed, returning development mock data`
              );
  ...
  const checkSpendPermission =
    useCallback(async (): Promise<SpendPermission | null> => {
      const token = getAuthToken();
  ...
  const authorizeSpending = useCallback(
    async (
      amount: number,
      signature: string
      ...
      try {
        ...
        if (response.success) {
          toast.success(
            `✅ Successfully authorized $${amount} CDP spending limit!`
          );
          return {
            authorized: true,
            allowance: amount,
  ...
  const checkPaymentEligibility = useCallback(
    async (amount: number, isUsingCredit: boolean = false) => {
      const token = getAuthToken();
      if (!token || !isAuthenticated) {
        ...
        return {
          eligible: false,
          payment_method: "none",
          recommendations: ["Please authenticate first"],
        };
      }
      ...
      try {
        ...
        if (response.success && response.data) {
          ...
          return {
            eligible: false,
            payment_method: "none",
            recommendations: ["Payment eligibility check failed"],
          };
        }
      } catch (error) {
        ...
        return {
          eligible: false,
          payment_method: "none",
          recommendations: ["Error checking payment eligibility"],
        };
      }
    },
    [apiCall, getAuthToken, isAuthenticated]
  ...
  const processPayment = useCallback(
    async (
      ...
      try {
        ...
        console.log("💳 Processing payment with cb_hack database:", {
          receiverAddress,
          amount,
          cashbackRate,
          isUsingCredit,
        ...
        if (response.success) {
          console.log(
            "✅ Payment processed successfully with cb_hack database:",
            response.data
          );
          return {
            success: true,
            ...
            cashback_earned:
              response.data.cashback_earned_rozo ||
              response.data.cashback_earned ||
              0,
  ...
  const retryWithBackoff = useCallback(
    async (operation: () => Promise<any>, maxRetries: number = 3) => {
